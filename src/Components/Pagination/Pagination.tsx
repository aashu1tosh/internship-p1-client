...
const Pagination: React.FC<PaginationProps> = ({ pagination, setTotalPages, setRefresh }) => {
        ...
        <div className='pagination'>
            ...
            <div className='pagination-row'>
                {getPaginationItems().map((pageNumber, index) => (
                    <span
                        key={index}
                        onClick={() => pageNumber !== '...' && handleClick(pageNumber as number)}
                        className={pageNumber === currentPage ? "underline-site-color margin-15" : "margin-15"}
                    >
                        {pageNumber}
                    </span>
                ))}
